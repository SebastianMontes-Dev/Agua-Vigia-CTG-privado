 ...
 * scripts/lib/datos-proyecto.mjs. Este archivo solo se encarga de inyectar
 * esos datos en la plantilla HTML y escribir el archivo. Se genera en local
 * (dist-dashboard/, ignorado por git); no se publica.
 *
...
function main() {
  ...
  console.log(`  Avance del proyecto: ${datos.avanceProyecto.porcentaje}% (${datos.avanceProyecto.sprintsCerrados}/${datos.avanceProyecto.sprintsTotal} sprints cerrados)`);
...
// control muestra "sin bugs registrados" con 70 bugs en el archivo. Estas advertencias son la unica
// senal de que el formato cambio.
function avisarDeSeccionesVacias(datos) {
  ...
  console.warn("   La página se generó igual, pero esas secciones salen vacías o incompletas.\n");
}
