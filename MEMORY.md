...
**Lección que se conserva:** verificar antes de afirmar. La afirmación no verificada casi cuesta la
mejor fuente de datos del proyecto.
...
**Decisión del proyecto: se respeta sin excepción**, aunque un colector con `User-Agent` propio no
...
nivel que ningún polígono de barrio puede representar. **Relevante para M9 y el dominio `Sector`**:
el matching texto→polígono necesita tolerancia y un umbral de confianza más bajo para avisos a
...
- **Meta (Facebook/Instagram):** CrowdTangle cerró en agosto de 2024. La Instagram Basic Display API
  ...
  Vía legítima: **Meta Content Library**, acceso académico vía ICPSR, que exige afiliación
  institucional. Puede tardar semanas y puede no aprobarse.
- **X/Twitter:** el nivel gratuito de la API no permite búsqueda de publicaciones. El plan pagado
  excede el presupuesto del proyecto. Fuera de alcance, documentado.
...
| Fecha | Decisión |
|---|---|
| 2026-08-06 | La fase de documentación va primero. No se escribe código de la aplicación hasta autorización explícita. |
...
| 2026-08-07 | Toda implementación, bug y sesión de trabajo con IA se registra en `docs/gestion/`. Es parte de la definición de terminado. Ver `ADR-008`. |
...
| 2026-08-07 | **El agente nunca figura como colaborador del repositorio**: sin `Co-Authored-By`, sin firmas en commits o PRs. La autoría es mía. Forzado con `includeCoAuthoredBy: false` y el workflow `autoria.yml`; en la nube, `git config user.*` con la identidad del dueño antes del primer commit: la de fábrica es `Claude` (`BUG-104`). Ver `CLAUDE.md` § Convenciones de Git. |
