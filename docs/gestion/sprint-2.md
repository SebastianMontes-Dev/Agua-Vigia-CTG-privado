...
|---|---|---|
| RF005–RF008 | ✅ Entregado — `POST /api/reportes` expone `RegistrarReporteService`. `@Cacheable` sobre `GET /api/sectores` y las reglas de rate limiting (`/api/veedor/sesion`, `/api/reportes/**`) ✅ ya en `main` (`SectorMongoAdapter`, `application.yml`) | Dominio y puertos ✅ · `RegistrarReporteService` ✅ (Sprint 1) · PR #104 |
...
| RF013 (completo) · RF015 | ✅ Entregado — `/api/suscripciones/confirmar` y `/cancelar` (desde `ADR-054` el `GET` solo muestra un botón y la acción es un `POST`), probados extremo a extremo (confirmar dos veces no falla, token inválido → 400 real) | Dominio y puertos ✅ · `SuscribirseService` ✅ (Sprint 1, PR #78) · PR #107 |
| RF008 (frontend) | 🟡 Retirado por alcance el 2026-09-21 (`ADR-048`: el frontend salió de `main` y se rehace en otras ramas de este repositorio; el contrato `POST /api/reportes` sí está entregado y documentado en `docs/api/reportes.md`). Antes: ✅ Implementado en `fix/integrar-formulario-reportes`, pendiente de revisión y fusión — `FormularioReporte` consume `POST /api/reportes`, genera una huella anónima SHA-256 estable, permite coordenada opcional y solo confirma éxito después del `201` real. Sin fallback ni datos simulados | `POST /api/reportes` ✅ (este sprint) |
...
| Rate limiting Redis (`RateLimitingInterceptor`) | PR #60 (Sprint 1) | ✅ reglas activas en `main` |
| Caché sobre Redis (`@EnableCaching`) | PR #61 (Sprint 1) | ✅ en uso en `main` |
...
retiró del proyecto el 2026-08-12**, al fusionar el rediseño de frontend a `main`, y **se retomó después**
(hay registros de esa carpeta con fecha de agosto y septiembre). Este sprint se abrió el 2026-08-09, tres días antes
