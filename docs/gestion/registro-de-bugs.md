...
| BUG-100 | 2026-09-22 | S1 | M15 | `AdministrarCuentaService.exigirQueQuedeUnAdministrador` lee el conteo de ADMIN activos y escribe la cuenta afectada en dos pasos separados, sin exclusión mutua entre peticiones: dos suspensiones o despromociones concurrentes sobre dos ADMIN distintos pueden leer el mismo "quedan 2" antes de que ninguna escriba, y las dos pasan la guarda a la vez — dejando el sistema sin ningún administrador activo, algo que la propia clase documenta como "no es un error recuperable desde la aplicación, hay que ir a Mongo a mano" | Cerrado — causa raíz: comprobar-y-actuar (`contarActivosPorRol` + `usuarios.guardar`) sin nada que serialice esas dos peticiones entre sí; Mongo en instancia única (sin replica set) tampoco permite resolverlo con una transacción multi-documento. Corrección: `BloqueoDeAdministradoresPort` (`ADR-062`) — bloqueo de exclusión mutua sobre un documento único de Mongo (`findAndModify` atómico, mismo patrón que `SectorMongoAdapter.cambiarEstadoSiEs`), con vencimiento de 10s si el proceso muere con el bloqueo tomado. `suspender()` y `cambiarPermisos()` ejecutan el conteo y el guardado dentro de `ejecutarExclusivo`; si no se puede adquirir, no se toca la cuenta y se puede reintentar. Pruebas: `BloqueoDeAdministradoresMongoAdapterTest` (8, con Mongo real: adquisición, rechazo mientras esté vigente, liberación, expiración, `ejecutarExclusivo` en éxito y en fallo) y `AdministrarCuentaServiceTest` (3 nuevas). Verificado: build completa, 853 pruebas, 0 fallos |
| BUG-101 | 2026-09-24 | S2 | M10 | En una instalación limpia con `docker compose up`, `POST /api/reportes/{id}/foto` responde 500 (`AccessDeniedException` en `data/fotos/…`): el volumen `fotos-data` nace como `root` y el usuario `aguavigia` del contenedor no puede escribir en él. Se rodea con un `chown` manual dentro del contenedor | Cerrado — hallado en la Fase 5 al recorrer los flujos HTTP contra un entorno construido desde cero (`scripts/verificar-flujos.mjs`). Causa raíz: el `Dockerfile` no creaba `/app/data/fotos` (el comentario suponía que `AlmacenamientoLocalAdapter` la crea al arrancar), y Docker solo copia dueño y permisos a un volumen con nombre nuevo si el directorio ya existe en la imagen; al no existir, el punto de montaje lo crea el motor como `root`. Un volumen ya creado con la imagen anterior sigue siendo de `root`: hay que borrarlo o hacer `chown` una vez. Corrección: `RUN mkdir -p /app/data/fotos && chown -R aguavigia:aguavigia /app` en `backend/Dockerfile`. Prueba: el paso «adjuntar una foto» de `verificar-flujos.mjs` (500 antes, 200 después, sobre volumen nuevo) y un paso de `despliegue-ci.yml` que falla si `/app/data/fotos` no existe en la imagen o no es de `aguavigia` (comprobado contra la imagen anterior: `stat` no encuentra el directorio) |
| BUG-102 | 2026-09-25 | S3 | M15 | `GET /api/veedor/usuarios?estado=PENDIENTE_APROBACION` publica en `Link` un enlace a la siguiente página sin el filtro `estado`: un cliente que sigue `Link` pasa de la cola de altas a la página 2 de todas las cuentas | Cerrado — hallado al agregar los filtros de la bitácora. Causa raíz: `CabecerasDePaginacion` armaba `Link` solo con `pagina` y `tamano`, sin forma de recibir los filtros de la petición. Corrección: sobrecarga de `CabecerasDePaginacion.respuesta` que añade al enlace los filtros no nulos, codificados; la usan la bitácora y el listado de cuentas. Prueba: `AdminUsuariosControllerTest#elEnlaceALaSiguientePaginaDeCuentasDebeConservarElFiltroDeEstado` y `BitacoraControllerTest#elEnlaceALaSiguientePaginaDebeConservarLosFiltros` |
| BUG-103 | 2026-09-25 | S3 | — (frontend) | El muestrario mostraba «Botón principal NaN:1» en el tema claro: la razón de contraste medida en pantalla no salía para `--sobre-cardenillo` | Cerrado — hallado por la E2E de la migración a la identidad de `ADR-071`. Causa raíz: el CSS que genera Vite acorta `#ffffff` a `#fff` y `getComputedStyle` devuelve esa forma; `contraste.ts` solo leía `#rrggbb`, así que `parseInt` de un canal vacío daba `NaN`. No se había visto porque ningún token anterior tenía forma corta. Corrección: `contraste.ts` expande `#rgb` a `#rrggbb`. Prueba: `contraste.test.ts#debeAceptarElHexadecimalCortoQueDevuelveElNavegador` y `muestrario.spec.ts` (fila «Botón principal» con 7,25:1 y 9,39:1) |
| BUG-105 | 2026-09-25 | S2 | M2 | La guía de escalabilidad exige una cola offline contraria a ADR-044 | Cerrado — reproducción: leer «Reportar sin hacer daño»; causa: recomendación antigua sin actualizar tras ADR-044; corrección: guía autoral (no generada) exige fallo explícito y reintento manual; comprobación: no hay instrucción de guardar pendientes en docs/api/escalabilidad-para-el-cliente.md |
| BUG-106 | 2026-09-25 | S2 | API | OpenAPI tipa varios errores de foto, confirmación e histórico como éxito | Cerrado — reproducción: foto/confirmar 400 y 404, cortes/sustento 404 contra Docker; todos application/problem+json. Causa: ApiResponse sin Content hace que Springdoc infiera el retorno exitoso; corrección: anotaciones ProblemDetail en 3 controladores y contrato regenerado (solo 6 respuestas); prueba: ContratoOpenApiTest.losErroresPublicosDeReporteEHistorialDebenDeclararProblemDetail |
| BUG-107 | 2026-09-25 | S2 | CI | api:check falla en Windows sin cambios de contrato | Cerrado — reproducción: checkout con core.autocrlf=true y api:check; causa: esquema.ts se convertía a CRLF mientras el generador escribe LF; corrección: .gitattributes fija LF en TS/TSX/CSS; prueba: api:sync no produce diff semántico y api:check pasa |
| BUG-109 | 2026-09-25 | S2 | M1 | El ejemplo de backoff de la guía puede esperar 90 s pese al tope de 60 s | Cerrado — reproducción: base saturada y aleatorio=1; causa: jitter multiplicado después de Math.min; corrección: tope después de jitter; prueba: canal-en-vivo.test.ts acota a sesenta segundos |
| BUG-110 | 2026-09-25 | S2 | M1/M2 | El formulario real en preview recibe 403 CORS pese a pasar el humo de consola | Cerrado — reproducción: abrir /sectores/bocagrande en 4173 y elegir reporte; causa raíz: changeOrigin traducía Host pero mantenía Origin de preview no permitido por Spring; corrección: traducir solo Origin del mismo host de la SPA; orígenes ajenos siguen bajo CORS; prueba: ciudadano.spec.ts reporta y confirma desde navegador |
| BUG-111 | 2026-09-25 | S2 | M1/M2 | MapLibre aparece sin polígonos con Worker failed to load | Cerrado — reproducción: abrir build de producción antes de fijar worker; causa raíz: MapLibre 6 separa worker ESM; Vite no lo empaqueta desde la URL predeterminada; corrección: import worker&url y setWorkerUrl local; prueba: ciudadano.spec.ts exige fuente y mapa visibles sin recursos externos |
| BUG-112 | 2026-09-25 | S3 | M1/M2 | Ficha sin encabezado principal y navegación desbordada en 768 px | Cerrado — reproducción: abrir ficha en escritorio y mapa en 768 px; axe y scrollWidth; causa raíz: se ocultó h1 del mapa sin promover el barrio; cinco destinos no cabían en tableta; corrección: h1 del barrio, historial h2 y menú compacto bajo 1100 px; prueba: revision.spec.ts axe y ausencia de scroll en siete anchos |
| BUG-113 | 2026-09-25 | S3 | M1/M2 | Zoom duplicado y controles blancos en tema oscuro | Cerrado — reproducción: abrir mapa en tema oscuro después de cargar CSS diferido; causa raíz: CSS nativo de MapLibre sin capas prevalecía sobre los estilos propios; corrección: controles propios de 44 px con tokens y botones con etiqueta; prueba: revision.spec.ts objetivos, axe y capturas de ambos temas |
| BUG-114 | 2026-09-25 | S2 | M1/M2 | El mapa tarda 5 segundos en mostrar estados en 3G fría | Cerrado — reproducción: CDP 1,6 Mbps, subida 750 Kbps, latencia 150 ms, caché desactivada; causa raíz: la representación dependía de descargar motor y worker antes de dibujar geometría; corrección: respuesta SVG progresiva y módulos bajo demanda ADR-075; prueba: rendimiento.spec.ts cuenta 211 polígonos y exige menos de 3000 ms |
| BUG-115 | 2026-09-25 | S2 | M1/M2 | El fixture de frescura no modificaba los sectores reales | Cerrado — reproducción: preparar fixtures y abrir zona-industrial/Alameda; causa raíz: confusión entre id público y _id Mongo, y entre campos DTO y Documento; corrección: filtrar por slug, escribir estadoActualizadoEn/estadoVerificadoEn y exigir matchedCount; prueba: revision.spec.ts exige null, censo ausente y 48 h reales |
| BUG-116 | 2026-09-25 | S3 | M1/M2 | Confirmación tiene un enlace de salto sin destino | Cerrado — reproducción: abrir /confirmar/no-existe y ejecutar axe; causa raíz: main carecía de id contenido y el salto quedaba fuera del landmark; corrección: salto dentro de header y main enfocable en todas las rutas; prueba: revision.spec.ts axe en confirmación en ambos temas y siete anchos |
| BUG-117 | 2026-09-25 | S2 | M2 | Primer reporte sin red intenta descargar el formulario y no explica el fallo | Cerrado — reproducción: cargar mapa, cortar red antes de abrir la hoja; causa raíz: lazy import solo se solicitaba al abrir y el mensaje offline vivía dentro del módulo inaccesible; corrección: comprobar conexión antes de abrir y mostrar fallo explícito con reintento manual; prueba: revision.spec.ts conexión offline antes del primer reporte |
| BUG-118 | 2026-09-25 | S3 | M2 | Foto inválida pide elegir el barrio a mano | Cerrado — reproducción: respuesta 400 peticion-invalida al subir foto; causa raíz: mensaje compartido con el reporte geolocalizado; corrección: contexto de foto con explicación de formatos y reporte conservado; prueba: ciudadano.test.ts foto inválida |
| BUG-119 | 2026-09-25 | S2 | M2 | Después de enviar con teclado, Escape no cierra el reporte | Cerrado — reproducción: abrir reporte con Enter y enviar con teclado; causa raíz: desaparece el botón enfocado al mostrar agradecimiento y el foco cae a body, fuera del diálogo; corrección: enfocar encabezado del resultado y restaurar explícitamente el disparador al cerrar; prueba: ciudadano.spec.ts envía, cierra con Escape y exige foco restaurado; recorrido completo adicional solo con teclado |
| BUG-120 | 2026-09-25 | S2 | M1/M2 (CI) | CI pierde el fixture de verificación de 48 horas | Cerrado — reproducción: arranque limpio y suite real, corrida 36217272293, 9/10; causa raíz: ingesta inicial del backend puede confirmar el estado después de sembrar fixtures y refrescar verificadoEn; local tenía la ingesta inicial terminada; corrección: reservar tarea-unica:ingesta en Redis antes de arrancar Spring y conservar reserva al vaciar caché; prueba: job real completo con estado antiguo, sin sustituir API, consenso ni SSE |
| BUG-108 | 2026-09-25 | S2 | M1 | Las portadas devuelven 404 por el proxy de Vite | Cerrado — reproducción: GET /acuacar-media/2026/09/AH-00071-300x200.jpg en dev/preview; causa: se enviaba a Spring, que no sirve esa ruta; corrección: proxy acotado a uploads con identificación y sin Referer, igual que nginx (ADR-038); prueba: verificar-proxy-real.mjs devuelve 200 image/jpeg en ambos puertos |
| BUG-104 | 2026-09-25 | S3 | — (proceso) | El squash del PR #57 llegó a `main` (`752806c`) con el trailer `Co-authored-by: Claude <noreply@anthropic.com>`, contra la regla de autoría de `CLAUDE.md`, aunque `includeCoAuthoredBy` está en `false` | Cerrado — hallado al registrar la fusión del #57. Causa raíz: la identidad git por defecto de las sesiones en la nube es `Claude <noreply@anthropic.com>`; el commit `aeb510b` de la rama se hizo con ella y no se pudo reescribir (force-push bloqueado), y GitHub agrega al squash un `Co-authored-by` por cada autor distinto de los commits del PR. `includeCoAuthoredBy` solo controla el trailer que escribe el agente, no la identidad con la que firma. El historial de `main` no se reescribe. Corrección: identidad del dueño en el `git config` local de la sesión y `scripts/verificar-autoria.sh`, que corre en `.github/workflows/autoria.yml` sobre los commits de cada PR. Prueba: el script falla contra el rango real del #57 (`567ab57..1ff007c`, señala `aeb510b`) y contra `752806c` (trailer), y pasa contra `0bc06da..567ab57` |
...
Siguiente número disponible: BUG-121
-->
